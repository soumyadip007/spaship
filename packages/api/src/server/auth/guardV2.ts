...
import { Target } from '../permission/entity';

@Injectable()
export class AuthenticationGuardV2 extends AuthGuard('jwt') {
  constructor(
  ...
  private async isAuthorized(requesterEmail: string, resource: string, method: string, propertyIdentifier: string, context: ExecutionContext) {
    ...
    if (access.criteria.includes('env')) target.env = env;
    if (access.criteria.includes('applicationIdentifier')) target.applicationIdentifier = applicationIdentifier;

    Object.keys(target).forEach((key) => (target[key] === undefined || target[key] === '') && delete target[key]);
    const authorized = (await this.dataServices.permission.getByAny({ email: requesterEmail, action: access.name, target }))[0];
    if (!authorized)
